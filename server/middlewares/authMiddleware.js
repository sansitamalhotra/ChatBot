...
const requireLogin = async (req, res, next) => {
  try {
    ...
    // IMPORTANT FIX: Fetch the current user from database
    ...
    // Set req.user to the database user object with _id field
    req.user = {
      _id: user._id,
      ...
      country: user.country,
      appliedJobs: user.appliedJobs,
      jobsPostedBy: user.jobsPostedBy || [], // Ensure array exists
      status: user.status,
      registeredDate: user.registeredDate,
      isVerified: user.isVerified,
      isBlocked: user.isBlocked,
    ...
    console.log("User set in req.user:", {
      ...
      role: req.user.role,
      jobsPostedByLength: req.user.jobsPostedBy ? req.user.jobsPostedBy.length : 'undefined'
    });
...
const isAdmin = async (req, res, next) => {
  try {
    // Use req.user._id since we now set it properly in requireLogin
    ...
    // Since we already fetched user in requireLogin, we can use req.user.role directly
    if (req.user.role === 1) {
      console.log("isAdmin: Access granted for admin user");
      next();
    } else {
      console.log("isAdmin: Access denied - user role is", req.user.role, "but admin requires role 1");
      return res.status(403).json({
        success: false,
...
const isSuperAdmin = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 3) {
      ...
      console.log("isSuperAdmin: Access denied - user role is", req.user.role, "but super admin requires role 3");
      return res.status(403).json({
        success: false,
        message: "ACCESS DENIED. You do not have the Authorization to View this Page!!!"
      });

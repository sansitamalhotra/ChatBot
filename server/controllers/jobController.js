...
const { sendJobNotificationEmail } = require('../services/SendNewJobNotification');


/**
 * Utility functions to improve reusability and code organization
 */
const buildPaginationResult = (page, limit, startIndex, endIndex, totalCount) => {
  const result = {};
...
module.exports = {
    ...
    postJob:async (req, res) => {        
        try {
            ...
            const newJobData = { title, slug, description, qualification, workExperience, workMode, filePath, country, province, sector, jobPostDate, jobPostedById, deadlineDate };
    ...
    try {
        ...
        const totalAppliedApplicant = await ApplicantJobApplication.countDocuments(query);

        // Ensure search is a string
        const searchQuery = search && typeof search === 'string' ? { title: { $regex: search, $options: "i" } } : {};
    ...
    fetchJobsByRecruiter: async (req, res) => {
        ...
        try
        {
            const totalJobs = await Job.countDocuments({ jobPostedById: user, deadlineDate: { $gte: new Date() } });

            numJobsInDB = await Job.countDocuments({ jobPostedById: user, deadlineDate: { $gte: new Date() } });

            const jobs = await Job.find({ jobPostedById: user, deadlineDate: { $gte: new Date() } })
        ...
        {
            console.log(error);
            res.status(500).send({ success: false, message: "Something Went Wrong Trying to Fetch All Jobs From Database", error: error.message });
        }
    },
    ...
    fetchSingleJobController: async (req, res) => {
        try {
          ...
          .populate("jobPostDate")
          .populate("jobPostedById")
          .populate("deadlineDate").exec();
          if (!job) return res.status(404).send({ success: false, message: "Job not found." });
          res.status(200).send({ success: true, message: "Job Details Found", job });
        } catch (error) {
          console.log(error);
          res.status(500).send({ success: false, message: "Something Went Wrong Trying to Fetch This Job From Database", error });
        }
    },
    ...
    deleteJobController: async (req, res) => {
        try 
        {
            ...
            const { slug } = req.params;
            const deletedJob = await Job.findOneAndDelete({ slug: slug }).select("-filePath");

            user.jobsPostedBy.pop(deletedJob._id);
            await user.save();   
            res.status(200).send({ success: true, message: "Job deleted successfully", deletedJob});
        } 
        catch (error) {
            console.log(error);
            res.status(500).send({ success: false,  message: "Error while deleting Job", error});
        }
    },
    ...
    updateJobController: async (req, res) => {
        ...
        try {  
            ...
            console.log(error);
            res.status(500).json({ success: false, message: "Error in updating Job", error: error.message });
        }
    },
    ...
    fetchJobBySlugController: async (req, res) => {
        ...
        try
        {
            ...
            const totalAppliedApplicant = await ApplicantJobApplication.countDocuments({ job: job._id }); 

            // console.log("Job Applicantion Status: ", applicationStatus);
            console.log("Job Applicantions: ", totalAppliedApplicant);

            res.status(200).send({ success: true, message: "Job Details Found", job, totalAppliedApplicant });
    ...
    filterJobsBySector: async (req, res) => {
        ...
        try {
            const sectorIds = await Job.distinct('sector', { deadlineDate: { $gte: new Date() } }).sort("-createdAt").exec();

            const totalSectors = await Sector.countDocuments({ _id: { $in: sectorIds } });
            const numSectorsInDB = await Sector.countDocuments({ _id: { $in: sectorIds } });
    
            const sectors = await Sector.find({ _id: { $in: sectorIds } }).limit(limit).skip(startIndex);
    ...
    fetchJobRelatedSectors: async (req, res) => {
        ...
        try {
            ...
            const sectors = await Sector.find({ _id: { $in: sectorIds } }).skip(skip).limit(limit);
...
filterJobsByNumber: async (req, res) => {
    ...
    {            
        ...
        const totalJobs = await Job.countDocuments(query);
        const numJobsInDB = await Job.countDocuments(query);
        const totalAppliedApplicant = await ApplicantJobApplication.countDocuments();
    ...
    {
        console.log(error);
        res.status(500).json({ error: "Something Went Wrong While Filtering Jobs By Number" });
    }
},
 /**
  ...
  fetchAllAppliedJobsByApplicants: async (req, res) => {
    try {
      ...
      const result = buildPaginationResult(page, limit, startIndex, endIndex, totalAppliedApplicant);
...
/**
  ...
  fetchRegisteredUsers: async(req, res) => {
    try {
      ...
      // Build pagination result
      const result = buildPaginationResult(page, limit, startIndex, endIndex, totalRegUsers);
  ...
  fetchAllPostedJobs: async (req, res) => {
    try {
      ...
      // Build pagination result
      const result = buildPaginationResult(parsedPage, parsedLimit, startIndex, endIndex, totalJobs);
